...
% Solve the inverse problem using the chosen function
lambda_s = 0.1e-8; % Spatial regularization parameter 0.1e-8 for L2 remember
lambda_t = 0.1e-8; % Temporal regularization parameter 0.1e-8 for L2 remember
tol = 3e-5; % Convergence tolerance  3e-5 for L2 remember
max_iter = 1500; % Maximum iterations ma tanto non ci arriva con L2, converge prima
rho = 50;      % ADMM penalty parameter

% Spatial Laplacian (Nsources x Nsources)
% dipole_positions is a Nsources x 3 matrix containing the positions of the dipoles
...
%edit of the 25_3, only the first solve_inverse_problem is provided, as the latters still are work in progress
...
threshold = 0.35 * max(J_reconstructed(:)); % Adjust threshold with some turbodisel

% Map source indices to dipole indices
% Since each dipole has 3 orientations, divide by 3 and round up idk why, it works
...
% Loop through each time point and red the active dipoles, black the off ones
for t = 1:T
...
% Plot for the forward problem (same as inverse, but use Xsimulated)
figure;
hold on;
...
% Define a threshold for active dipoles
threshold = 0.01 * max(Xsimulated(:)); % Adjust threshold
