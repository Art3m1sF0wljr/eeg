...
active_sources = 15; % Number of active sources
...
% Calculate pairwise Euclidean distances between sources
source_positions = DipoleField.pos(good_dipoles, :);
distances = pdist2(source_positions, source_positions);

% Initialize active sources at the first time step
% Select 1 random dipole and 5 others in close spatial proximity
initial_dipole = randperm(length(good_dipoles), 1); % Randomly select 1 dipole
[~, closest_indices] = sort(distances(initial_dipole, :)); % Find closest dipoles
active_indices = closest_indices(1:active_sources); % Select the closest 5 dipoles

% Ensure active_indices is a column vector
active_indices = active_indices(:);

% Assign initial activation values
Xsimulated(active_indices, 1) = rand(active_sources, 1) .* time_smooth(1);

% Simulate source activity over time with smooth transitions
for t = 2:T
    % Randomly select one active dipole to turn off
    turn_off_index = randsample(active_indices, 1);
    
    % Remove the turned-off dipole from the active_indices list
    active_indices = setdiff(active_indices, turn_off_index);
    
    % Find a new dipole to turn on, in close spatial proximity to the remaining active dipoles
    % Calculate the mean position of the remaining active dipoles
    mean_position = mean(source_positions(active_indices, :), 1);
    
    % Calculate distances from the mean position to all dipoles
    distances_to_mean = pdist2(source_positions, mean_position);
    
    % Exclude already active dipoles
    distances_to_mean(active_indices) = Inf; % Set distance to active dipoles to Inf
    
    % Select the closest inactive dipole
    [~, new_dipole] = min(distances_to_mean);
    
    % Ensure new_dipole is a column vector
    new_dipole = new_dipole(:);
    
    % Add the new dipole to the active_indices list
    active_indices = [active_indices; new_dipole];
    
    % Assign activation values to the new active dipoles
    Xsimulated(active_indices, t) = rand(active_sources, 1) .* time_smooth(t);
end

% Spatial smoothness: smooth source activity over space
space_smooth_factor = 0.8; % Spatial smoothness factor (higher means more smoothing)
