...
% Base probability of a source lighting up
base_probability = 0.001;

% Initialize active sources at the first time step
active_indices = randperm(length(good_dipoles), active_sources); % Randomly select 6 sources
...
for t = 2:T
    % Calculate the center of mass (barycenter) of the currently active sources
    barycenter = mean(source_positions(active_indices, :), 1);
    
    % Calculate distances from the barycenter to all sources
    distances_to_barycenter = pdist2(source_positions, barycenter);
    
    % Calculate probabilities of lighting up based on distance from barycenter
    probabilities = base_probability ./ (1 + distances_to_barycenter); % Inverse distance weighting
    
    % Normalize probabilities to ensure they sum to 1
    probabilities = probabilities / sum(probabilities);
    
    % Select new active sources based on the calculated probabilities
    new_active_indices = randsample(length(good_dipoles), active_sources, true, probabilities);
    
    % Ensure the new active sources are unique
    new_active_indices = unique(new_active_indices);
    
    % If fewer than active_sources are selected, add more randomly
    while length(new_active_indices) < active_sources
        additional_indices = randsample(length(good_dipoles), active_sources - length(new_active_indices), true, probabilities);
        new_active_indices = unique([new_active_indices; additional_indices]);
    end
    
    % Random activation for active sources
    temp_activity = rand(active_sources, 1);
    % Apply temporal smoothing
    Xsimulated(new_active_indices, t) = temp_activity .* time_smooth(t);
    
    % Update active_indices for the next time step
...
tol = 1.7e-5; % Convergence tolerance
