...
% Compute pairwise Euclidean distances between dipoles
dipole_positions = DipoleField.pos(good_dipoles, :); % Positions of all dipoles (N x 3)
distances = pdist2(dipole_positions, dipole_positions); % Pairwise distances (N x N)

% Define spatial proximity threshold (e.g., 20 mm)
spatial_threshold = 20; % Adjust as needed

% Simulate source activity with spatial and temporal proximity
time_smooth = exp(-linspace(-2, 2, T).^2); % Gaussian envelope for smooth variation

% Randomly select a starting dipole
starting_dipole = randi(Nsources); % Randomly select a starting dipole

% Simulate activations
for t = 1:T
    % Find dipoles within the spatial proximity threshold of the starting dipole
    nearby_dipoles = find(distances(starting_dipole, :) <= spatial_threshold);
    
    % Randomly select 'active_sources' dipoles from the nearby dipoles
    active_dipoles = nearby_dipoles(randperm(length(nearby_dipoles), min(active_sources, length(nearby_dipoles))));
    
    % Generate random activations for the selected dipoles
    temp_activity = rand(length(active_dipoles), 1); % Random activation for active sources
    Xsimulated(active_dipoles, t) = temp_activity .* time_smooth(t); % Apply temporal smoothing
    
    % Update the starting dipole for the next time point (to ensure temporal proximity)
    starting_dipole = active_dipoles(randi(length(active_dipoles))); % Randomly select one of the active dipoles
end
...
lambda_s = 0.1e-6; % Spatial regularization parameter
lambda_t = 0.1e-6; % Temporal regularization parameter
...
for t = 1:T
    ...
    title(sprintf('Active Dipoles at Time Point %d/%d', t, T));
...
% Define a threshold for active dipoles
threshold = 0.5 * max(Xsimulated(:)); % Adjust threshold as needed
