...
% Randomly select active dipoles for each time point
for t = 1:T
    % Randomly select 'active_sources' dipoles
    active_dipoles = randperm(Nsources, active_sources); % Randomly select dipoles
...
threshold = 0.1 * max(J_reconstructed(:)); % Adjust threshold as needed
...
% Define a threshold for active dipoles
threshold = 0.3 * max(Xsimulated(:)); % Adjust threshold as needed
